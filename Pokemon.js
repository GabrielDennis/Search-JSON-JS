...
//This below function is to display all the information of the given pokemon

function getpokemon(poke_name){
  for (i=0; i<pokemonDetails.pokemon.length; i++){        // Looping through the entire length of the JSON Variable
    if(pokemonDetails.pokemon[i].name == poke_name){      // Search/Check for the given pokemon in the JSON Variable
      console.log(pokemonDetails.pokemon[i]);             // displaying all the aspects of the identified pokemon
    }
    else{
      continue;
    }
...
/*This below function is to list all the pokemons whose next evlolution is the same
as the given name*/

function evolutionFinder(evolution_name){
  for(j=0; j<pokemonDetails.pokemon.length; j++){                   // Looping through the entire JSON Variable
    if ('next_evolution' in pokemonDetails.pokemon[j]){             // Checking for next_evolution availability
      for(i=0; i<pokemonDetails.pokemon[j].next_evolution.length; i++){ // Iteration = number of evolution present
        if(pokemonDetails.pokemon[j].next_evolution[i].name==evolution_name){ // Check to find the matching evolution
          console.log("Evoloution : "+pokemonDetails.pokemon[j].name);    // Display the pokemons with same next_evolution stage
        }
        else{
          continue;
        }
...
//This below function is to list all the pokemons who share the same given weakness

function weakness_Shared(weakness){
  for(i=0; i<pokemonDetails.pokemon.length; i++){         // Looping through the entire JSON Variable
    if('weaknesses' in pokemonDetails.pokemon[i]){        // Checking the presence of weakness in a pokemon
        var arr = pokemonDetails.pokemon[i].weaknesses;   // If there is any weakness, assigning that into a variable for easy access
        for (weaks in arr){                               // Looping through the obtained weakness
          if(arr[weaks]==weakness){                       // Check/Search for the matching weakness
            console.log(weakness+" as Weakness : "+pokemonDetails.pokemon[i].name);   //Display all the pokemons who share the same weakness
          }
          else{
            continue;
          }
...
var name = "Mew";
var weak = 'Bug';
var evolve = "Dragonite"

//Function Call with arguments
getpokemon(name);
evolutionFinder(evolve)
weakness_Shared(weak)
